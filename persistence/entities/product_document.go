...
// Formula: 0.3 * SNELL + 0.1 * ECE + 0.1 * DOT + 0.5 * (SHARP-Left-Percentage * 0.20 + SHARP-Right-Percentage * 0.20 + SHARP-TopFront-Percentage * 0.20 + SHARP-TopRear-Percentage * 0.20 + SHARP-Rear-Percentage * 0.20)
...
func (p ProductDocument) CalculateSafetyPercentage() int {
	...
	// ECE is the minimum standard required for helmet use in the UK, and helmets must be proven to meet this standard before being sold in the UK (not based on the honor system!)
	if p.Certifications.ECE {
		totalScore += 0.15
	}

	// DOT is pretty much useless since it's based off the honor system, hence a 0.05 weight
	...
	if p.Certifications.SHARP != nil {
		...
		totalScore += float64(0.5) * (sharpImpactWeight*(float64(sharpImpacts.Left)/sharpImpactMaxValue) +
